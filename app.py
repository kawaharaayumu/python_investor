...
with st.expander("📖 グラフの見方・用語解説"):
    ...
    - **本体（太い部分）**: **「始値（朝の価格）」と「終値（夕方の価格）」**の差を表します。
        - **緑色の本体（陽線）**: 下端が始値、上端が終値です。
        - **赤色の本体（陰線）**: 上端が始値、下端が終値です。
    - **ヒゲ（上下に伸びる細い線）**: その日の**「高値」と「安値」**を示します。
        - 本体から上に突き出ているのが高値、下に突き出ているのが安値です。
        - **長い下ヒゲ**: 一時大きく売られたが、最後は大きく買い戻された「反発の兆し」。
    ...
    ### 4. ローソク足と出来高の色が違う時
    同じ日でも、色が食い違うことがあります。これは**「比較対象」が違うため**です。
    - **ローソク足**は、その日一日の「朝 vs 夕方」を比較しています。
    - **出来高の棒**は、前日と比べた「昨日 vs 今日」を比較しています。
    - **例：** 前日よりは安いけれど、朝の安値から猛烈に買い戻された日は、**出来高は「赤（前日比マイナス）」でも、ローソク足は「緑（当日内は上昇）」**になります。これは「下げ止まり」のサインとして注目されることがあります。
